<div>
	...
	foreach ($list as $row) {
		if($row['flag'] == 0){
		?>
		<div class="notif notif-waiting" onclick="showDetailAdmin(<?php echo $row['id_notif']; ?>)">
		<?php
		}
		else if($row['flag'] == 1){
		?>
		<div class="notif notif-accepted" onclick="showDetailAdmin(<?php echo $row['id_notif']; ?>)">
		<?php
		}
		else if($row['flag'] == 2){
		?>
